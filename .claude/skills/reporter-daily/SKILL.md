...
以下の順番で並べる（1以外は該当するものだけ記載）：

1. `・note 執筆`（無条件で毎回トップに記載。よーんが確認して書いていない日はこの行を削除して最終版にする運用のため、実際に書いたかは判定しない）
2. **X投稿**（`x_posts_today` が1件以上ある場合のみ見出しを出す）
3. **threads 投稿**（`threads_posts_today` が1件以上ある場合のみ見出しを出す）
4. note 記事公開（`note_publish_today` がある場合。見出しなし）
5. 変更ログまとめ（`changelog_entries` がある場合。見出しなし）
6. 投稿以外の活動（見出しなし。1活動1項目）
