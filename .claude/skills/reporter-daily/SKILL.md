...
## 投稿の特記事項フォーマット
以下のいずれかの形式で書く（投稿時刻・ツイート種類はテーマで代替）：

- **テーマあり**: `・[投稿種類]投稿（[テーマ]）：[数値]。[一言]`
  - 例: `・ワンポイント解説投稿（宇宙では爆発音が聞こえない）：インプ3,892・いいね119・引用10・ブクマ8。GW中でも定期投稿を維持できた。`
- **テーマ不要**: `・[投稿種類]投稿。[数値]。[一言]`
  - 例: `・GW帰省投稿。インプ382・いいね23。琵琶湖の風つよかった。`

## 一言の選び方（最重要）
- **数値に対する自分の感想は書かない**
  - NG例: 「119いいねはちょっと嬉しい」「思ったより伸びた」「日常系も意外と来た」「ブクマ多くて驚いた」
- 残してよい一言は次の2種だけ：
  1. **投稿内容に関連する体験談**（例：「琵琶湖の風つよかった」）
  2. **運用上の事実報告**（例：「GW中でも定期投稿を維持できた」）
- 上記2種に該当しなければ一言を書かない（無理に毎項目につけない）

## 用語の統一
- 「ツイート」「ポスト」ではなく **「投稿」** に統一
- 投稿時刻（朝6時／昼／夜など）は書かない
- 数値は `インプXXX・いいねXXX・引用XXX・ブクマXXX` の順で具体的に（**RT→引用、BM→ブクマ**）
