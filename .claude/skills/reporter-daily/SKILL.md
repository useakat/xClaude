...
`ポスト数` は**引用を含んだ件数**なので、オリジナルポスト数は `originals = posts − quotes` で求める（例: ポスト数3・引用数2 → オリジナルポスト数1）。

このシートの数値は **X の実績のみ**（threads は含まない）。threads の件数は STEP 5 で別に数える。

行が見つからない場合は `posts`・`quotes`・`replies`・`originals` を空として続行する。
...
**`ポスト種類` が `引用RT` の行はオリジナルポストと分け、`x_quotes_today` として記憶する。** 引用は STEP 8 で `引用` の項目の下に内訳を書き、オリジナルポストの内訳には入れない。引用にはラベル（型）を付けないので、outputs との突合も不要。

残りの行（通常ポスト）はポストURLを `outputs_today` のURL列と突合し、一致すればその what_id を種類ラベル対応表でラベルに変換する。一致しなければラベルは「その他」。

インプレッションが最も大きい投稿を `x_top` として記憶する（STEP 8 で「一言」を書くかどうかの参考情報として使う）。

抽出・分類したオリジナルポストの一覧を `x_posts_today` として記憶する。
...
- **オリジナルポスト数には引用を含めない。** `posts` ではなく `originals`（= `posts − quotes`）を書く
- `x_quotes_today` が1件以上あれば、`引用` の下に内訳を全件、全角スペース2個の入れ子で列挙する（フォーマットは下記「引用の内訳」）
- `x_posts_today` が1件以上あれば、`オリジナルポスト数` の下に内訳を全件、全角スペース2個の入れ子で列挙する（**リプライ・引用は STEP 4 で除外済み**）
- 件数と内訳の件数が食い違う場合は、**件数表示は日次記録シートの値を正とし、内訳は投稿一覧の全件を出す**
...
```
　　・[ラベル]（[投稿内容を10字程度で要約]）：[数値]
```

一言を添えるときだけ、数値のあとを「。」で区切って書く：
...
### 同じ型が2件以上あるとき（型のグルーピング表記）

**同じラベル（型）の投稿が2件以上あるときは、型を見出し行にして、その下にもう1段インデントした内訳を並べる。**
...
　　　　・地球型惑星はありふれている：views214・いいね3・リポスト0・引用0・リプ0

　　　　・SLIM三度目の目覚め：views114・いいね1・リポスト0・引用0・リプ0
```

- **見出し行（`　　・[ラベル]`）にはテーマの括弧書きを付けない。** テーマは1段下の各行に、ラベル・括弧なしで書く（引用の内訳と同じ書き方）
- **同じ型の投稿が1件だけのときは、これまで通り `　　・[ラベル]（[テーマ]）：[数値]` の1行のみ**（見出し＋入れ子にしない）
- note販促用などの注記が必要な投稿が入れ子内にある場合は、その行のテーマの前に注記を置く（例: `　　　　・note販促用、ボイジャー1号を救った物語：views12・いいね1・リポスト0・引用0・リプ0`）
- 同じ型の中でのインプ／views 大きい順の並びは、この入れ子内でも適用する
...
- **「その他」などのラベルも括弧も付けない。** 要約をそのまま項目名にする
  - 例: `　　・ひので20年感謝：インプ609・いいね15・リポスト2・ブクマ0・リプ1`
- 数値の並びと末尾の「。」のルールはオリジナルポストと同じ

## 内訳の並び順（X・threads 共通）

- **オリジナルポスト（X・threads）は型ごとにまとめて書く。** 同じラベルの投稿は続けて並べる。**同じ型が2件以上あるときは「同じ型が2件以上あるとき（型のグルーピング表記）」の入れ子形式にする**（1件のときは1行のまま）
- **型どうしの並びは次の順で固定する**：長文ストーリー → ワンポイント解説 → 質問回答 → 短文 → その他
- **インプ（threads は views）の大きい順に並べるのは、同じ型の中だけ**（型をまたいでインプ順に並べ替えない）
- **引用は型の区別が無いので、引用全体をインプの大きい順に並べる**
...
5. **引用の分離**: 引用の内訳が `引用` の下にあり、オリジナルポストの内訳に混じっていないか。オリジナルポスト数が `posts − quotes` になっているか
6. **引用の書き方**: 引用の内訳に「その他（〜）」などのラベルや括弧が付いていないか
7. **末尾の「。」**: 一言の無い内訳の行が「。」で終わっていないか
8. **並び順**: オリジナルポストが型ごとにまとまり（長文ストーリー → ワンポイント解説 → 質問回答 → 短文 → その他）、同じ型の中でインプ／views の大きい順になっているか。引用はインプの大きい順になっているか
9. **型のグルーピング表記**: 同じ型が2件以上あるとき、`　　・[ラベル]` の見出し＋1段深い入れ子（テーマ・括弧なし）になっているか。1件だけの型が誤って見出し＋入れ子にされていないか
...
```
✅ 日報作成完了: [DATE_JP]
   X: オリジナルポスト数 [originals] / 引用 [quotes] / リプライ [replies]
