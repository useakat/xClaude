...
# STEP 1.5: スプレッドシート連携の準備確認

STEP 2 に進む前に、ToolSearch で `mcp-gsheets sheets_get_values` を検索し、スキーマが返ってくることを確認する（これによりサーバーが利用可能になるまで待機される）。スキーマが返ってきたら即座に STEP 2 へ進む。
...
最新１０行の開始行: `start = max(2, N - 9)`（1-indexed）
...
- `ツイート種類`・`ツイート本文`（先頤80字）・`インプレッション`・`いいね`・`リツイート`・`ブックマーク`
...
3. Xストーリー投稿
4. note 記事投稿
5. 変更ログまとめ（changelog_entries がある場合）
6. その他

## 自己チェック（保存前に必ず実施）

生成した特記事項を STEP 6 で保存する前に、以下を1項目ずつ声に出して確認する。１つでも引っかかったら書き直す。
...
**重要**: 各項目・各箇条書きの間には必ず空行を１行入れること。空行がないと Wiki で改行が無視される。
