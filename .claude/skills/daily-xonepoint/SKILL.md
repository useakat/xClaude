...
返却された以下を記憶し、STEP 3 へ進む：
- **【タイトル案】**: タイトル候補10件
- **【本文】**: 投稿原稿
- **【ネタNo】**: writer-xonepoint が本文末尾に添えた「No.X」のネタ番号

さらに、使用したネタの **分野（K列）** を **【分野】** として記憶する。STEP 1 で取得済みの onePointNeta データから【ネタNo】に一致する行を特定し、K列（分野）の値を読み取る（K列が空欄の場合は「その他」とする）。
...
STEP 3 で記憶した【タイトル案】【最終原稿】【チェックサマリー】、STEP 2 で記憶した【分野】を使用して、`mcp__claude_ai_Gmail__create_draft` ツールで Gmail 下書きを作成する。

1. 以下の形式で本文を組み立てる：
   ```
   [ネタ番号]

   onePointNeta[（STEP 2 で記憶した【ネタNo】）]
