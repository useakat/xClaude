...
4. **`image-plan.md` を解析する。** `## ` 行で H2 セクションに分割し、各セクションの絞り込まれた 1 案から **画像種類**（図解 / イメージ / 写真）と **画像説明** を抽出する。`# 画像プランニング` 見出しは無視する。

5. **写真画像はスキップ。** 画像種類が「写真」のセクションは生成せず、「スキップ: <セクション>（写真）」と記録する。
...
## <H2タイトル2>（イメージ画像）
- draft/images/<safe>_イメージ_01.png
- draft/images/<safe>_イメージ_02.png
- draft/images/<safe>_イメージ_03.png
