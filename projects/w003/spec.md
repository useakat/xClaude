...
9.5. **フック型の記録** — `/writer-xpost` の冒頭フック候補で採用した型（`style/hook-patterns.md` の①〜⑤。複合は主たる型を1つ）を `logs/hook_records.csv` に1行追記する。形式: `日付(JST),W003,{①〜⑤+短縮名 例:②知識提示},{投稿文の冒頭15字},onePointNeta[{No}],,{謎},{自分事}`（冒頭15字は投稿文の先頭15文字そのまま・半角カンマは含めない。週次振り返りが X投稿一覧の本文と突合するキー）。謎=「あり・{答えを知りたくなる問いを一言}」/「なし」、自分事=「あり・{読者の生活との接点を一言}」/「なし」（制作時の自己申告・半角カンマ不可。2026-09-21 追加）。追記はテーマフォルダの通常コミットに含める
