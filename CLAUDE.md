...
- `git push` 完了を報告する際は、push 先ブランチ名を明示する（例：「master に push しました」）
...
- **よーんに承認を求める質問をした後は、よーんの明示的な返事があるまで次のアクション（commit・push など）に進まない。Stop hook など hook のフィードバックが入っても、それを承認の代わりとみなして実行してはならない**
