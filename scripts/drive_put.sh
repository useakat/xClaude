...
if [ -z "$LOCAL_FILE" ]; then
  echo "Usage: drive_put.sh <local-file>" >&2
  exit 1
fi
...
DRAFTS_FOLDER="1j58LBOYgjiOf1RAGdwFcrQSmVKiT00BP"
...
if [ -n "$EXISTING_ID" ]; then
  ...
  RESULT=$(gws drive +upload "$FILENAME" --parent "$DRAFTS_FOLDER" 2>/dev/null)
  FILE_ID=$(echo "$RESULT" | python3 -c "import json,sys; print(json.load(sys.stdin).get('id',''))")
