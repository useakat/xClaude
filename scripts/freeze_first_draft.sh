...
#   (引数なし) … Stop / UserPromptSubmit から。記録簿にある draft.md だけを凍結して記録簿を空にする。
#
# Stop（＝ユーザーに原稿を提示した瞬間）まで待ってから凍結するのは、w002 のように
# 執筆後に自動チェックを重ねてから提示するフローがあるため。書き込み時点では凍結しない。
#
# Stop に加えて UserPromptSubmit でも同じ凍結処理を呼ぶのは、ターンが中断されると
# Stop が発火しないケースがあるため（2026-08-15 判明）。UserPromptSubmit はよーんが
# 次の発言を送った時点で必ず発火するので、Stop の取りこぼしを回収する保険になる。
# 二重に走っても、first-draft.md が既にあれば何もしないため無害。
#
