...
- **X投稿のインプ監視を新設（投稿から3時間以内に1,000インプでGmail通知）** — 伸び始めた投稿に投稿直後に気づく手段が無かった（円周率バズではリプの6割が初動6〜12時間に集中したのに、リプ返しの中央値は32.6時間後）。X API でオリジナル投稿＋引用RTのインプを10分おきに確認し、閾値到達で1投稿1回だけメール通知する cron（`x_imp_alert.py`）を追加。実送信テスト済み。[→報告書](../reports/20260926_x_impression_alert/)
- **W002 spec にフロー15.5（公開版 output/index.md の生成）を新設し、フロー15以降の確認を省略可に** — `output/index.md` を作る工程が spec に定義されておらず（モードB前提で書かれていた）、note 下書き保存の直前で不在に気づいた。draft から公開版を生成する6手順を明文化し、フロー15以降はユーザー確認なしで実行してよい旨も追記。[→報告書](../reports/20260926_w002_output_index_flow/)

## 2026-09-25

- **日報の引用・並び順・行末句点ルールを reporter-daily に反映** — 9/24 の日報への手直し4点をスキルに固定。日次記録の `ポスト数` は引用込みのため、オリジナルポスト数を `ポスト数−引用数` とし、引用RTは `引用` の下にラベル無しで書く。内訳の行末「。」は一言があるときだけ。並びはオリジナルポストを型ごと（長文ストーリー→ワンポイント解説→質問回答→短文→その他）にまとめ、**インプ順は同じ型の中だけ**、引用は全体をインプ順。[→報告書](../reports/20260925_reporter_daily_quote_order_rules/)
- **reporter-daily の照合漏れを修正（outputs の日付形式・X の URL 表記の違い）** — outputs の日時はハイフン区切り、X の URL は outputs が `x.com`・X投稿一覧が `twitter.com` 表記で、どちらも日報スキルの照合と食い違い、**全投稿の型が判定できず「その他」になる**状態だった。日付はハイフン区切りで、URL は末尾の投稿番号で照合するよう修正（週次振り返りスキルの方式にそろえた）。[→報告書](../reports/20260925_reporter_daily_outputs_match_fix/)
- **X分析ダッシュボード（Xダッシュボード タブ）を「発信記録」スプレッドシートに新設** — カード12枚＋推移グラフ4枚（週次/月次は日付指定に関わらず直近12週・12ヶ月を固定表示）を追加。mcp-gsheets では設定できないデータ検証・グラフの積み上げ/色指定は GAS `setupXDashboardExtras()` を新設して反映した。[→報告書](../reports/20260925_x_dashboard_setup/)

## 2026-09-21

- **check-yohn W001 ペルソナに W9 拡張＋W10〜W13 を追記（指示語の限定・時間接続語・視点往復・余白・サムネ明るさ）** — アポロ12号 note 販促（2回目）で check-yohn を2周通過した原稿に、よーんから27件の指摘。うち判断基準として言語化できる5系統を、いつ直すかが決まる**判断テスト＋実例**の形でペルソナ化した。当初案の「接続語は4つで足りる」は「そうとも限らない」と撤回され、関係の種類（同時／直後／逆接／転換）ごとの例示にとどめた。あわせて outputs シートの W001 販促記録3件を整備。[→報告書](../reports/20260921_check_yohn_w001_reference_time_viewpoint/)
- **H5 字数基準を「本文のみ（タグ・改行・空白除く）」に統一し遡及分析を更新、hook_records に謎・自分事列を追加** — シート文字数列との約60字のずれで W001 の帯判定が揺れていたため基準を統一（ルナ9号は本文536字＝500字帯 n=1 に再判定）。新基準の再計算で「短いほど良い」の単調性は再現せず「本文700字超は一貫して弱い」のみ確認、前向き検証は継続。あわせてフック型ラベルで説明できない伸びの差（型・題材が同じで36倍差）に対応し、「謎」「自分事」の制作時自己申告列を hook_records.csv と3プロジェクトの記録手順に追加。[→報告書](../reports/20260921_h5_charcount_unification/)
- **note 新ダッシュボードの GraphQL API からインプレッション・記事別売上・流入元を取得** — ダッシュボード刷新で新指標が REST `/api/v1/stats/pv`（旧ダッシュボードの API）から取れなくなっていた。JS チャンクからクエリを抽出して `graphql.note.com` を特定し、**POST `/api/v3/graphql/auth` で JWT を取得し Authorization: Bearer で叩く2段構えの認証**（Cookie だけでは未ログイン扱い）を実装。`note投稿一覧` に K〜M列（インプレッション・コメント・売上）を追加し、**H〜J列の出所も REST から GraphQL に一本化**（累計が 7.9% ずれ、同一期間ならほぼ一致・スキ・コメントは完全一致。GraphQL が全履歴を持つため段差なく全28行を再構築）。`note流入元` シートを新設し387日分をバックフィル。[→報告書](../reports/20260921_note_dashboard_graphql_api/)

## 2026-09-19

- **check-yohn W001 ペルソナに W8「短文の連打」・W9「主語の省略」を新設し、W2 に「オチの先出しは咎めない」を追記** — ルナ9号長文でフロー6.8 を3周通過した原稿に、フロー7 でよーんから13往復の指摘。うち3件をペルソナ化した。**W2 は共通コア判断基準10「種明かしの先出しをしない」の W001 例外**で、検品役の構成指摘を受けて writer が冒頭を差し替えたところ、よーん本人が元の「すっぱ抜いてしまった話」に戻した事例。W001 の冒頭2文目は結末を伏せる場所ではなく落差を最大化する場所だという判断。W8 は **brand.md A-3「転換では短文を畳みかける」と衝突する**ため検品役が自力で判断できなかった箇所で、「短文が効くのは転換の合図1行だけ／説明・描写の連なりを短文で刻むのは別物」と切り分けた。[→報告書](../reports/20260919_check_yohn_w001_rhythm_and_subjects/)

## 2026-09-03

- **check-yohn W001 ペルソナに「締めで本文の語を再利用するとぎこちなくなる」を追記** — ルメートル販促投稿で、check-yohn を3周通過した締め「一度『正しい』と認められた計算は〜」によーんから「イマイチ」の指摘。この締めは冒頭フックの台詞「あなたの計算は正しい」と呼応させたもので、**W1 の「冒頭の具体物と呼応して輪を閉じると強い」に従った結果**だった。問題は論理ではなく言い回し（カギカッコ付き引用の再利用・「認められた計算」という硬い名詞句）で、W1 には輪の閉じ方の**失敗**パターンが無く検出できなかった。成功条件の直後に失敗例を並べ、指針を「呼応させるのは『意味』であって『語』ではない」に凝縮した。[→報告書](../reports/20260903_check_yohn_w001_closing_phrase/)

## 2026-08-31

- **フック型記録 `logs/hook_records.csv` を導入（z01/W003/W001 の制作フローに記録ステップ追加）** — 3週連続未達だったフック型記録を「人がメモする」から「制作フローが決定時に CSV へ自動追記する」設計に変更。冒頭15字を突合キーに週次振り返りが型別分析に使う。z01 は無人フローのため CSV の即 commit & push まで spec に明記。[→報告書](../reports/20260831_hook_records_csv/)
...
- **W001 の高反応投稿の型を実績8本から抽出して3層ルール化** — 採点基準は品質の下限を担保していたが「実際に伸びた投稿が何をしていたか」は未整理で、`hook-patterns.md` の5型も並列に並ぶだけだった。高反応8本を帰納分類したところ**フックは⑤口火型6本・①逆説型2本に完全集中**（②③④型の高反応は0本）。8つの特徴を出現率で3層に分け（A必須4=8/8本／B原則2=6〜7本／C題材依存2=4〜6本）、全項目必須にすると題材に合わないスパイスを強いるため層を分けた。⑤口火型は温度形容（ヤバい／震える／痺れる）なしの実績が無い点も明記。[→報告書](../reports/20260829_w001_high_performance_post_patterns/)
- **W001 の画像フローを lovart に追従させ、年代考証と生成のコツを明文化** — spec のフロー10 が nano banana のままで、既に lovart へ移行済みの実運用（W002は8/9・W003は8/15）から取り残されていた。あわせて実制作で判明した2点を追加：①**題材の年代と参照写真の年代の食い違い**（ハッブルのパネルは1990〜93年が金色の初代型、2002年SM3B以降が濃紺。当時の実写は至近距離のものしかなく「時代の正確さ」と「理想の構図」が両立しないため、1990年実写=色の正解／2009年実写=構図の参照と役割を分けて解決）②**修正を重ねると別の箇所が崩れる**（参照画像を毎回添付しても発生。lovart が自分の前回出力を編集するため）→ 崩れたら新規スレッドで一発生成し直す。[→報告書](../reports/20260829_w001_lovart_image_flow/)
