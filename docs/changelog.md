...
- **X長文の字数基準を「本文のみ」に統一し、hook_records に謎・自分事列を追加** — 週次振り返り H5 の字数が X投稿一覧の「文字数」列（タグ・改行込み、本文より中央値 +60字）に依存しており、brand.md の採点基準と別物になっていた。本文のみ（ハッシュタグ行・改行・空白を除く）に統一して遡及分析を再計算した結果、「短いほど良い」は再現せず**「本文700字超は一貫して弱い」のみ確認**。フック型だけでは伸びを説明できないため、制作時に謎・自分事を自己申告する列を3プロジェクトの記録手順に追加した。[→報告書](../reports/20260921_x_char_count_rule_and_hook_records_columns/)
