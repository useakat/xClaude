...
- **check-yohn の W003 ペルソナにファクト解釈の見逃しパターンを追記（T9〜T12）** — クマムシ弾丸実験の制作で、フロー5〜6.5 の全チェックを通過した原稿によーんの指摘が4件出た。最も深刻だったのは、notebook の要約が論文の**肯定的な例外**（地球起源イジェクタの43%は月に生存可能な速度で衝突する）を落としており、その要約だけで「答えはほぼ否」と結論を組み立てていたこと。修正しようとして今度は**問いの射程**（惑星間）と**答えの数字**（地球→月）がずれた。あわせて `/check-critic` の未裏取り指摘（大気減速）を確定事実として報告した件（論文に `atmosphere` の語は一度も無く、大気は一切考慮されていなかった）、指示語「この数十倍」が直前の速度を指すのに実際は圧力の話だった件を、T9〜T12 として追記。T11 には「支持／否定／**記載なし**」の3判定を明記した。[→報告書](../reports/20260829_check_yohn_w003_fact_interpretation/)

## 2026-08-23

- **reporter-daily の日報全文表示ステップを強化（ツール結果を表示とみなさない）** — 8/21分の日報 routine が STEP 9.5（全文表示）を実質スキップした実例（Read で読んだことを表示とみなした・前回実行の引き継ぎで残作業だけ完了させた）を受け、「Read/cat のツール結果は表示とみなさず回答本文にコードブロックで全文を貼る」「引き継ぎ実行でも省略しない」の2ルールを STEP 9.5 に明記。[→報告書](../reports/20260823_reporter_daily_display_rule/)
...
- **W002 にサスペンス設計ルールを新設（物語の疑問と理解の疑問の区別・感情の設計図）** — 「正確で分かりやすいが面白くない」というユーザー診断への対応。原因は説明量ではなく、**制作系全体が誤解ゼロに最適化され全ての疑問がその場で説明される（サスペンスが残らない）**構造にあると診断し、オポチュニティ冬ごもり章の Before/After 実験（場面主導版）で方針確認のうえルール化。理解の疑問（因果・仕組み）は即答／**物語の疑問（成否・正体）は意図的に保留して引きにする**と扱いを分け、brand.md に運用ルール4点（説明は質問の後・問いと答えを密着させない・章の頭は場面から・賭け金は前例の記憶で）、spec.md の agenda に「感情の設計図」（保留リスト＋回収章・感情の線・1場面・一次証言の有無確認）を新設。check-reader / check-yohn には「設計どおりの保留を未回収として潰さない」連携を配線した（読者役の隔離は維持し writer 側で選別）。[→報告書](../reports/20260820_w002_suspense_design_rules/)
