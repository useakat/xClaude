...
| H | 出典メモ | 参照した情報源 |
| **I** | **ステータス** | **未使用 / 使用済み** |
| J | 追加日 | ネタを追加した日（`YYYY-MM-DD`） |
| K | 分野 | 宇宙 / 物理 / 化学 / 生物 / 医学 から1つ |
...
| **L** | **ステータス** | **未使用 / 使用済み** |
| M | 追加日 | 追加日 |
| N | 短文最終使用日 | z01（X短文投稿）で最後に使用された日（`YYYY-MM-DD`）。z01 のネタ選定時にステータス≠ボツかつ空欄／90日以上前の行を候補とするために参照する |
...
| **G** | **ステータス** | **未使用 / 使用済み** |
| H | 追加日 | 追加日 |
| I | 短文最終使用日 | z01（X短文投稿）で最後に使用された日（`YYYY-MM-DD`）。noteNeta の N列と同じ用途 |
