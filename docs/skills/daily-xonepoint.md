...
check-tonmana は `projects/x-onepoint/brand.md` に照らして文体・口調・構成のみ調整する（事実は変更しない）。

返却された以下を記憶する：
- **【調整後本文】**: トンマナ調整後の本文
- **【トンマナサマリー】**: check-tonmana が返したトンマナ調整1行

## 4-3: P01化スコアリング

4-2 の【調整後本文】を引数として `/check-p01` スキルを呼び出す。
check-p01 は P01化チェックリスト6項目を採点し、全項目8点以上になるまで該当箇所を書き直す（事実は変更しない）。

返却された以下を記憶して STEP 5 へ進む：
- **【最終原稿】**: 合格した（または最高得点の）本文
- **【スコアサマリー】**: check-p01 が返したスコアサマリーテーブル

### 4-3-1: チェックサマリーへの記録

【チェックサマリー】の末尾に、check-tonmana の【トンマナサマリー】1行と check-p01 が返した【スコアサマリー】の各行（P01化-項目1〜6／P01化-合計）を追記して記憶し直す。
check-p01 が **不合格（警告）** を返した場合は、メール本文の冒頭にも警告行を入れる：
