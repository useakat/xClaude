...
- **すべての箇条書きの間に空行を1行入れる。** 空行が無いと Wiki で改行が無視され、全項目が1行に連結される

## note セクション

- 1行目は無条件で `・記事執筆中` と書く（進捗の詳細は、よーんが後から括弧書きで追記する運用。**スキル側で推測して書かない**。よーんが書いていない日はこの行を削除して最終版にする）
- `note_publish_today` があれば、次の項目として記事公開を追加する：

```
・記事公開：[記事内容の要約]の[価格]円有料記事。[当日の販売状況があれば1文]
```

例: `・記事公開：Xストーリー投稿の深堀りの980円有料記事。タイムセール中（490円）に4記事売れた。`
...
`x_posts_today` が1件以上あれば、`オリジナルポスト数` の下に内訳を全件、全角スペース2個の入れ子で列挙する。
`posts` の件数と `x_posts_today` の件数が食い違う場合は、**件数表示は日次記録シートの値を正とし、内訳は投稿一覧の全件を出す**。
...
- ラベルが「その他」の投稿にも同じフォーマットを使う（例: `　　・その他（はやぶさ2が撮影した小惑星トリフネの画像）：インプ816・いいね60・リポスト10・ブクマ4・リプ0。`）
...
・[変更ログまとめ・投稿以外の活動（各1項目）]
```

**重要**:
- 各項目・各箇条書きの間には必ず空行を1行入れること。空行が無いと Wiki で改行が無視され、全項目が1行に連結される
